...
(function exposeLocalDb(root, factory) {
    const api = factory();
    if (typeof module === 'object' && module && module.exports)
        module.exports = api;
    if (root)
        root.MiniLocalDb = api;
    ...
    function normalizeAttendance(value) {
        ...
        for (const [date, records] of Object.entries(value)) {
            if (!isObject(records))
                throw new Error(`Invalid attendance day "${date}"`);
            const day = {};
    ...
    function readLegacySnapshot(storage) {
        ...
        if (parsedRequests !== null)
            snapshot.requests = parsedRequests;
        if (parsedTemplates !== null)
            snapshot.templates = parsedTemplates;
        return snapshot;
    }
    ...
    function normalizeSnapshot(value) {
        if ((value === null || value === void 0 ? void 0 : value.schemaVersion) !== 1 || !Array.isArray(value.users) || !isObject(value.attendance)) {
        ...
        if (typeof value.lastUpdateTimestamp !== 'string')
        ...
        if (Array.isArray(value.requests))
            normalized.requests = value.requests;
        if (Array.isArray(value.templates))
            normalized.templates = value.templates;
    ...
    async function requestPersistentStorage(navigatorLike) {
        var _a, _b;
        try {
            return await ((_b = (_a = navigatorLike === null || navigatorLike === void 0 ? void 0 : navigatorLike.storage) === null || _a === void 0 ? void 0 : _a.persist) === null || _b === void 0 ? void 0 : _b.call(_a)) || false;
        }
        catch {
            return false;
        }
    ...
    function createLocalDb(options) {
        const adapter = options.adapter || (options.indexedDB ? createIndexedDbAdapter(options.indexedDB) : null);
        async function readState() {
            if (!isEnabled(options.storage) || !adapter || !await adapter.readMarker())
                return null;
            const snapshot = await adapter.readSnapshot();
            return snapshot ? normalizeSnapshot(snapshot) : null;
        }
        async function writeState(value) {
            if (!isEnabled(options.storage))
                throw new Error('Local database is disabled');
            if (!adapter || !await adapter.readMarker())
