...
type IconStyle = 'emoji' | 'lucide';

interface IconDef {
  emoji: string;
  body: string; // inner SVG markup of a 24x24 Lucide icon
}

interface IconDescriptor {
  kind: 'emoji' | 'svg';
  value: string; // the emoji char, or the full <svg> markup
  name: string;
}

(function exposeIconSet(root: any, factory: () => unknown) {
  const api = factory();
  if (typeof module === 'object' && module && module.exports) module.exports = api;
  if (root) root.IconSet = api;
})(typeof globalThis !== 'undefined' ? globalThis : this, function createIconSetApi() {
  const ICON_STYLES: IconStyle[] = ['lucide', 'emoji'];
  const DEFAULT_ICON_STYLE: IconStyle = 'lucide';

  // Semantic name -> { emoji fallback, Lucide body }. Lucide v1.21.0 (ISC).
  const ICONS: { [name: string]: IconDef } = {
    ...
    pin: { emoji: '📌', body: '<path d="M12 17v5"/><path d="M9 10.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V16a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V7a1 1 0 0 1 1-1 2 2 0 0 0 0-4H8a2 2 0 0 0 0 4 1 1 0 0 1 1 1z"/>' },
    today: { emoji: '📆', body: '<path d="M8 2v4"/><path d="M16 2v4"/><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M3 10h18"/><path d="m9 16 2 2 4-4"/>' },
    requests: { emoji: '📝', body: '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" x2="8" y1="13" y2="13"/><line x1="16" x2="8" y1="17" y2="17"/><polyline points="10 9 9 9 8 9"/>' },
    star: { emoji: '⭐', body: '<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>' },
    starFilled: { emoji: '★', body: '<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" fill="currentColor"/>' },
    copy: { emoji: '📋', body: '<rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/>' },
    share: { emoji: '📤', body: '<circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" x2="15.42" y1="13.51" y2="17.49"/><line x1="15.41" x2="8.59" y1="6.51" y2="10.49"/>' },
    link: { emoji: '🔗', body: '<path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/>' },
    unlink: { emoji: '⛓', body: '<path d="M9.5 14.5 7 17a4 4 0 0 1-5.657-5.657l3-3A4 4 0 0 1 10 8"/><path d="m14 16 3-3a4 4 0 0 0-1-6"/><path d="m3 3 18 18"/>' },
    package: { emoji: '📦', body: '<path d="m7.5 4.27 9 5.15"/><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/>' },
    wrench: { emoji: '🔧', body: '<path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/>' },
    filter: { emoji: '🌪️', body: '<polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/>' },
    users: { emoji: '👥', body: '<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><path d="M16 3.128a4 4 0 0 1 0 7.744"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><circle cx="9" cy="7" r="4"/>' },
    pause: { emoji: '⏸️', body: '<rect width="4" height="16" x="6" y="4" rx="1"/><rect width="4" height="16" x="14" y="4" rx="1"/>' },
  ...
  const FALLBACK: IconDef = { emoji: '•', body: '<circle cx="12" cy="12" r="3"/>' };
  ...
  function normalizeIconStyle(style: unknown): IconStyle {
    return style === 'emoji' ? 'emoji' : DEFAULT_ICON_STYLE;
  }

  function getDef(name: string): IconDef {
  ...
  function iconMarkup(name: string, style?: unknown): string {
    return normalizeIconStyle(style) === 'emoji' ? iconEmoji(name) : iconSvg(name);
  }

  function resolveIcon(name: string, style?: unknown): IconDescriptor {
