type MiniScope = { ownerUid: string; siteId: string; sourceId: string };
type MiniAttendanceRow = {
...
type MiniEnvelope = {
  ...
  capturedAt: string;
  rows: MiniAttendanceRow[];
};
type MiniOutboxRecord = {
  eventId: string;
  envelope: MiniEnvelope;
  state: 'pending' | 'sending' | 'dead' | 'ack';
  attempts: number;
  nextAttemptAt: number;
  error?: string;
};

(function exposeMiniEnvelope(root: any, factory: () => unknown) {
  const api = factory();
  if (typeof module === 'object' && module && module.exports) module.exports = api;
  if (root) root.MiniSaEnvelope = api;
  ...
  function iso(value: unknown): string {
    ...
    if (new Date(result).toISOString() !== result) throw new Error('capturedAt must be ISO-8601');
    return result;
  }
  ...
  function normalizeScope(value: any): MiniScope {
    return {
  ...
  function normalizeEnvelope(value: any): MiniEnvelope {
    ...
    if (!Array.isArray(value.rows) || !value.rows.length) throw new Error('rows are required');
    const rows = value.rows.map((row: any, index: number): MiniAttendanceRow => {
      if (row?.status !== 'present') throw new Error(`rows[${index}].status must be present`);
  ...
  function createEnvelope(value: any, uuid?: () => string): MiniEnvelope {
    const eventId = value?.eventId || uuid?.() || globalThis.crypto?.randomUUID?.();
    return normalizeEnvelope({ ...value, schema: SCHEMA, eventId });
  }
  function createOutbox(
    store: { load(): Promise<MiniOutboxRecord[]>; save(records: MiniOutboxRecord[]): Promise<void> },
    options: { now?: () => number; maxAttempts?: number; baseDelayMs?: number } = {}
  ) {
    const now = options.now || Date.now;
    const maxAttempts = options.maxAttempts || 5;
    const baseDelay = options.baseDelayMs || 1000;
    async function enqueue(value: MiniEnvelope): Promise<MiniOutboxRecord> {
      const envelope = normalizeEnvelope(value);
      const records = await store.load();
      const existing = records.find(record => record.eventId === envelope.eventId);
      if (existing) return existing;
      const record: MiniOutboxRecord = {
    ...
    async function processNext(
      transport?: (envelope: MiniEnvelope) => Promise<{ eventId: string; acknowledged: boolean }>
    ): Promise<MiniOutboxRecord | null> {
      ...
      try {
        ...
        if (!ack.acknowledged || ack.eventId !== record.eventId) throw new Error('Invalid ACK');
        record = { ...record, state: 'ack' as const, error: undefined };
      } catch (error) {
        const message = error instanceof Error ? error.message : 'Transport failed';
        record = record.attempts >= maxAttempts
          ? { ...record, state: 'dead' as const, error: message }
          : {
            ...record,
