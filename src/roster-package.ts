type RosterScope = { ownerUid: string; siteId: string; sourceId: string };

(function exposeRosterPackage(root: any, factory: () => unknown) {
  const api = factory();
  if (typeof module === 'object' && module && module.exports) module.exports = api;
  if (root) root.MiniRosterPackage = api;
  ...
  function scope(value: any): RosterScope {
    return {
  ...
  function integrityChecksum(value: unknown): string {
  ...
  function body(value: any) {
    if (value?.schema !== SCHEMA) throw new Error(`schema must be ${SCHEMA}`);
    const generatedAt = text(value.generatedAt, 'generatedAt');
    if (new Date(generatedAt).toISOString() !== generatedAt) throw new Error('generatedAt must be ISO-8601');
    if (!Array.isArray(value.employees)) throw new Error('employees must be an array');
    const ids = new Set<string>();
    const employees = value.employees.map((employee: any, index: number) => {
      const id = text(employee?.id, `employees[${index}].id`);
      if (ids.has(id)) throw new Error(`duplicate employee id "${id}"`);
      ids.add(id);
      ...
      if (employee.position) result.position = text(employee.position, `employees[${index}].position`);
      return result;
    });
    return {
      schema: SCHEMA,
  ...
  function createRosterPackage(value: any) {
    const safeBody = body({ ...value, schema: SCHEMA });
    return Object.freeze({ ...safeBody, checksum: integrityChecksum(safeBody) });
  }
  function validateRosterPackage(value: any, expectedScope?: RosterScope, expectedVersion?: string) {
    const safeBody = body(value);
    if (value.checksum !== integrityChecksum(safeBody)) throw new Error('Roster checksum mismatch');
  ...
  function parseRosterPackage(raw: string, expectedScope?: RosterScope, expectedVersion?: string) {
