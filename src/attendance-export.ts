...
type AttExportScope = { ownerUid: string; siteId: string; sourceId: string };

interface AttExportSubmissionRow {
  miniLocalId: string;
  number: string;
  name: string;
  normalHours: number;
...
interface AttExportSubmissionEnvelope {
  schema: 'attendance-submission/v1';
  submissionId: string;
  saProjectId: string;
  scope: AttExportScope;
  deviceId: string;
...
interface AttExportRequestEnvelope {
...
interface AttExportResponseEnvelope {
  schema: 'attendance-response/v1';
  requestId: string;
  saProjectId: string;
  ok: boolean;
  fromDate?: string;
  toDate?: string;
  submissions?: AttExportSubmissionEnvelope[];
  error?: string;
}

interface AttExportGenerateSubmissionOptions {
  workDate: string;
  saProjectId: string;
  repository?: any;
...
interface AttExportHandlerContext {
  channel?: any;
  peer?: any;
  isChannelAuthenticated?: (channel: any) => boolean;
  expectedSaProjectId?: string;
  repository?: any;
  attendanceData?: Record<string, Record<string, any>>;
  employees?: any[];
  employeeRepository?: any;
...
(function exposeAttendanceExport(root: any, factory: () => unknown) {
  ...
  const DEFAULT_SCOPE: AttExportScope = {
  ...
  function checkExactKeys(value: unknown, allowed: string[], label: string): void {
  ...
  function normalizeAttendanceSubmissionId(value: unknown): string {
  ...
  function requireCanonicalSaId(value: unknown, label: string): string {
  ...
  function validateCalendarDate(value: unknown, label: string): string {
    ...
    if (month < 1 || month > 12) throw new TypeError(`${label} must be YYYY-MM-DD`);
    const daysInMonth = new Date(Date.UTC(year, month, 0)).getUTCDate();
    if (day < 1 || day > daysInMonth) throw new TypeError(`${label} must be YYYY-MM-DD`);
    const roundTrip = new Date(Date.UTC(year, month - 1, day)).toISOString().slice(0, 10);
    if (roundTrip !== text) throw new TypeError(`${label} must be YYYY-MM-DD`);
    return text;
  }
  ...
  function rangeDays(fromDate: string, toDate: string): number {
  ...
  function validateScope(value: unknown): AttExportScope {
  ...
  function validateRow(row: unknown, index: number): AttExportSubmissionRow {
  ...
  function validateAttendanceSubmission(
    value: unknown,
    expectedSaProjectId?: string
  ): AttExportSubmissionEnvelope {
    ...
    if (!UUID_RE.test(submissionId)) throw new TypeError('submissionId must be a UUID');
    ...
    const result: AttExportSubmissionEnvelope = {
    ...
    if (Object.prototype.hasOwnProperty.call(rec, 'clientSequence')) {
      ...
      result.clientSequence = rec.clientSequence as number;
    }

    if (Object.prototype.hasOwnProperty.call(rec, 'excludedCount')) {
      if (!Number.isSafeInteger(rec.excludedCount) || (rec.excludedCount as number) < 0) {
        throw new TypeError('excludedCount must be a safe integer >= 0');
      }
      result.excludedCount = rec.excludedCount as number;
    }
    ...
    if (Object.prototype.hasOwnProperty.call(rec, 'errorSummary')) {
      ...
      const err = rec.errorSummary as Record<string, unknown>;
      if (!Number.isSafeInteger(err.unparsedFragments) || (err.unparsedFragments as number) < 0) {
      ...
      result.errorSummary = {
        unparsedFragments: err.unparsedFragments as number,
        codes: [...(err.codes as string[])]
      };
    }
  ...
  function validateAttendanceRequest(raw: unknown): AttExportRequestEnvelope {
    if (!isPlainObject(raw)) throw new TypeError('attendance-request must be an object');
    ...
    const normalized: AttExportRequestEnvelope = {
  ...
  function generateAttendanceSubmission(
    options: AttExportGenerateSubmissionOptions
  ): AttExportSubmissionEnvelope | null {
    ...
    let dayRecords: Record<string, any> = {};
    ...
    } else if (typeof (globalThis as any).attendanceRepository?.getByDate === 'function') {
      dayRecords = (globalThis as any).attendanceRepository.getByDate(workDay) || {};
    } else if ((globalThis as any).attendanceData && typeof (globalThis as any).attendanceData === 'object') {
      dayRecords = (globalThis as any).attendanceData[workDay] || {};
    }
    ...
    } else if (Array.isArray((globalThis as any).users)) {
      employeesList = (globalThis as any).users;
    } else if (typeof (globalThis as any).employeeRepository?.getAll === 'function') {
      employeesList = (globalThis as any).employeeRepository.getAll() || [];
    }

    const empMap = new Map<string, any>();
    for (const emp of employeesList) {
      if (emp && emp.id) empMap.set(String(emp.id), emp);
    }

    const rows: AttExportSubmissionRow[] = [];
    let excludedCount = 0;
    ...
    for (const [empId, rec] of empEntries) {
      ...
      const row: AttExportSubmissionRow = {
        ...
        number: String(emp.number ?? '').trim() || String(emp.id).trim(),
  ...
  function generateAttendanceSubmissionsForRange(
    options: AttExportRangeOptions
  ): AttExportSubmissionEnvelope[] {
    if (!options || typeof options !== 'object') {
      throw new TypeError('Options must be provided');
    }
    ...
    const submissions: AttExportSubmissionEnvelope[] = [];
  ...
  function isChannelAuthSafe(channel: any, isAuthFn?: (ch: any) => boolean): boolean {
    if (!channel) return false;
    if (typeof isAuthFn === 'function') {
      try { return isAuthFn(channel); } catch { return false; }
    }
    if (typeof (globalThis as any).SaMiniP2P?.isChannelAuthenticated === 'function') {
      try { return (globalThis as any).SaMiniP2P.isChannelAuthenticated(channel); } catch { return false; }
    }
    if (typeof channel.isAuthenticated === 'function') {
      try { return channel.isAuthenticated(); } catch { return false; }
    }
  ...
  function handleAttendanceRequest(
    rawRequest: unknown,
    context: AttExportHandlerContext = {}
  ): AttExportResponseEnvelope {
    ...
    if (expectedProject === undefined || expectedProject === null) {
      if (context.peer && typeof (context.peer as any).saProjectId === 'string') {
        expectedProject = (context.peer as any).saProjectId;
      }
    }
    if (expectedProject !== undefined && expectedProject !== null) {
  ...
  function sendAttendanceResponse(
    channel: any,
    response: AttExportResponseEnvelope,
    isAuthFn?: (ch: any) => boolean
  ): void {
  ...
  function attachAttendanceResponder(
    channel: any,
    peer: any,
    context: AttExportHandlerContext = {}
  ): () => void {
    ...
    const handler = (event: any) => {
      try {
        ...
        if (!parsed || parsed.schema !== ATTENDANCE_REQUEST_SCHEMA) return;
        ...
        try {
          ...
          try {
            ...
            if (typeof p?.requestId === 'string') reqId = p.requestId;
            if (typeof p?.saProjectId === 'string') projId = p.saProjectId;
          } catch (_) {}
          if (reqId && channel && channel.readyState === 'open' && isChannelAuthSafe(channel, context.isChannelAuthenticated)) {
            ...
            const errResponse: AttExportResponseEnvelope = {
              schema: ATTENDANCE_RESPONSE_SCHEMA,
              requestId: reqId,
              saProjectId: projId,
              ok: false,
    ...
    return () => {
      try { channel.removeEventListener('message', handler); } catch (_) {}
    };
  }
