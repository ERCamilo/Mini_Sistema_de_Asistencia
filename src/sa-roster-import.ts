...
type SaEnvelopeSchema = 'sa-roster/v1';

interface SaRosterRecordInput {
  saProjectId?: unknown;
  saEmployeeId?: unknown;
  number?: unknown;
  name?: unknown;
  position?: unknown;
  sueldo?: unknown;
...
interface SaRosterEnvelopeInput {
  schema?: unknown;
  version?: unknown;
  saProjectId?: unknown;
...
interface SaNormalizedRoster {
  schema: SaEnvelopeSchema;
  version: 1;
  saProjectId: string;
  rosterVersion?: string;
  generatedAt?: string;
  employees: SaNormalizedRecord[];
}

interface SaLocalEmployee {
  id: string;
  name: string;
  number: string;
  position?: string;
...
interface SaReconciliationCandidate {
  saProjectId: string;
  saEmployeeId: string;
  number: string;
  name: string;
  candidates: Array<{ id: string; number: string; name: string }>;
}

interface SaImportPlan {
  ok: boolean;
  reason: 'needs-confirmation' | null;
  roster: SaNormalizedRoster;
  updates: Array<{ localId: string; record: SaNormalizedRecord }>;
  creates: SaNormalizedRecord[];
  reconciliationCandidates: SaReconciliationCandidate[];
  totalValid: number;
}

interface SaNumberRules {
  normalizeEmployeeNumber(value: unknown): number | null;
}

interface SaApplyOptions {
  generateId?: () => string;
  now?: () => string;
  normalizeEmployeeNumber?: (value: unknown) => number | null;
  confirmedLinks?: Array<{ saProjectId: string; saEmployeeId: string; localId: string }>;
}

interface SaApplyResult {
  users: SaLocalEmployee[];
  createdCount: number;
  updatedCount: number;
  linkedCount: number;
  skippedCount: number;
...
(function exposeSaRosterImport(root: any, factory: () => unknown) {
  const api = factory();
  if (typeof module === 'object' && module && module.exports) module.exports = api;
  if (root) root.SaRosterImport = api;
  ...
  function isValidSaId(value: unknown): value is string {
  ...
  function normalizeId(value: unknown, field: string): string {
    if (!isValidSaId(value)) throw new Error(`SA roster ${field} is malformed`);
    return (value as string).trim();
  }

  function defaultNormalizeNumber(value: unknown): number | null {
    if (value === null || value === undefined || String(value).trim() === '') return null;
    const normalized = Number(value);
    return Number.isFinite(normalized) ? normalized : null;
  }

  function recordHasSaSignal(record: unknown): boolean {
    return isObject(record) && (hasOwn(record, 'saEmployeeId') || hasOwn(record, 'saProjectId'));
  }

  function classifyImportPayload(parsed: unknown): { kind: 'sa' | 'legacy'; reason: string } {
    ...
    if (isObject(parsed)) {
      const employees = hasOwn(parsed, 'employees') ? (parsed as Record<string, unknown>).employees : undefined;
      const recordsHaveSignal = Array.isArray(employees) && (employees as unknown[]).some(recordHasSaSignal);
      if (
        (parsed as Record<string, unknown>).schema === SCHEMA ||
        (parsed as Record<string, unknown>).source === 'sa' ||
        recordsHaveSignal ||
      ...
      if (Array.isArray(employees)) return { kind: 'legacy', reason: 'employees object without SA signals' };
      return { kind: 'legacy', reason: 'object without SA signals' };
    }
    return { kind: 'legacy', reason: 'non-object payload' };
  }

  function normalizeSaRecord(raw: unknown, index: number, defaultProjectId: string): SaNormalizedRecord {
    if (!isObject(raw)) throw new Error(`SA roster employees[${index}] must be an object`);
    const record = raw as Record<string, unknown>;
    for (const key of Object.keys(record)) {
      if (RECORD_ALLOWLIST.indexOf(key) < 0) throw new Error(`SA roster employees[${index}] has unsupported field "${key}"`);
    }
    ...
    if (!number) throw new Error(`SA roster employees[${index}].number is required`);
    const name = trimmedText(record.name);
    if (!name) throw new Error(`SA roster employees[${index}].name is required`);

    const normalized: SaNormalizedRecord = { saProjectId, saEmployeeId, number, name };
    ...
    if (hasOwn(record, 'paused')) {
      if (typeof record.paused !== 'boolean') throw new Error(`SA roster employees[${index}].paused must be a boolean`);
      normalized.paused = record.paused as boolean;
    }
    return normalized;
  }
  ...
  function normalizeSaRoster(value: unknown): SaNormalizedRoster {
    if (!isObject(value)) throw new Error('SA roster payload must be an object with schema "sa-roster/v1"');
    const envelope = value as Record<string, unknown>;
    for (const key of Object.keys(envelope)) {
      if (ENVELOPE_ALLOWLIST.indexOf(key) < 0) throw new Error(`SA roster has unsupported field "${key}"`);
    }
    if (envelope.schema !== SCHEMA) throw new Error(`SA roster schema must be ${SCHEMA}`);
    if (envelope.version !== VERSION) throw new Error('SA roster version must be 1');
    const saProjectId = normalizeId(envelope.saProjectId, 'saProjectId');
    if (!Array.isArray(envelope.employees)) throw new Error('SA roster employees must be an array');
    ...
    if (hasOwn(envelope, 'rosterVersion') && envelope.rosterVersion !== undefined) {
      ...
      if (!text) throw new Error('SA roster rosterVersion is malformed');
      rosterVersion = text;
    }
    ...
    if (hasOwn(envelope, 'generatedAt') && envelope.generatedAt !== undefined) {
      ...
      const iso = (envelope.generatedAt as string).trim();
      if (new Date(iso).toISOString() !== iso) throw new Error('SA roster generatedAt must be ISO-8601');
      generatedAt = iso;
    }

    const rawEmployees = envelope.employees as unknown[];
    ...
    for (const employee of employees) {
      ...
      if (norm === null) throw new Error(`SA roster employees number "${employee.number}" is malformed`);
    ...
    const normalized: SaNormalizedRoster = {
      schema: SCHEMA,
      version: VERSION,
      saProjectId,
      employees
    };
    if (rosterVersion !== undefined) normalized.rosterVersion = rosterVersion;
  ...
  function buildSaImportPlan(
    ...
    const existingByTuple = new Map<string, SaLocalEmployee>();
    for (const user of existingUsers) {
      if (user.saProjectId !== undefined || user.saEmployeeId !== undefined) {
        ...
        if (existingByTuple.has(key)) throw new Error('Stored employees contain a duplicate SA identity');
        existingByTuple.set(key, user);
      }
    }
    const existingById = new Map<string, SaLocalEmployee>();
    for (const user of existingUsers) {
      if (!existingById.has(user.id)) existingById.set(user.id, user);
    }
    const existingByNumber = new Map<number, SaLocalEmployee[]>();
    for (const user of existingUsers) {
    ...
    const updates: SaImportPlan['updates'] = [];
    const creates: SaNormalizedRecord[] = [];
    const reconciliationCandidates: SaReconciliationCandidate[] = [];
    const incomingByTuple = new Map<string, SaNormalizedRecord>();
    for (const record of safeRoster.employees) {
      incomingByTuple.set(tupleKey(record.saProjectId, record.saEmployeeId), record);
    }
    const ownerMovesAway = (owner: SaLocalEmployee, targetNorm: number): boolean => {
      if (!isValidSaId(owner.saProjectId) || !isValidSaId(owner.saEmployeeId)) return false;
      ...
      if (exact) {
        ...
        if (recordNorm !== null) {
          ...
          if (blockers.length > 0) {
            ...
            throw new Error(
              `SA roster conflicting link: "${record.name}" [${record.saEmployeeId}] (${record.number}) collides with "${blocker.name}"`
            );
          }
        }
  ...
  function applySaRosterToUsers(
    existingUsers: SaLocalEmployee[],
    roster: SaNormalizedRoster,
    options: SaApplyOptions = {}
  ): SaApplyResult {
    const safeRoster = normalizeSaRoster(JSON.parse(JSON.stringify(roster)));
    ...
    const rawByTuple = new Map<string, Record<string, unknown>>();
    if (isObject(roster) && Array.isArray((roster as unknown as Record<string, unknown>).employees)) {
      for (const raw of ((roster as unknown as Record<string, unknown>).employees as unknown[])) {
        if (!isObject(raw)) continue;
        const rec = raw as Record<string, unknown>;
        const projectFallback = normalizeId((roster as SaNormalizedRoster).saProjectId, 'saProjectId');
        const project = hasOwn(rec, 'saProjectId') && typeof rec.saProjectId === 'string' && (rec.saProjectId as string).trim()
          ? (rec.saProjectId as string).trim()
          : projectFallback;
        if (typeof rec.saEmployeeId !== 'string') continue;
        rawByTuple.set(tupleKey(project, (rec.saEmployeeId as string).trim()), rec);
      }
    }
    // Also index the normalized input positionally as a fallback.
    const rawList: unknown[] = isObject(roster) && Array.isArray((roster as unknown as Record<string, unknown>).employees)
      ? ((roster as unknown as Record<string, unknown>).employees as unknown[])
      : [];

    function rawFor(record: SaNormalizedRecord, fallbackIndex: number): Record<string, unknown> | null {
      const direct = rawByTuple.get(tupleKey(record.saProjectId, record.saEmployeeId));
      if (direct) return direct;
      const positional = rawList[fallbackIndex];
      return isObject(positional) ? (positional as Record<string, unknown>) : null;
    }
    ...
    const candidateByTuple = new Map<string, SaReconciliationCandidate>();
    ...
    const confirmedByTuple = new Map<string, string>();
    for (const link of confirmedLinks) {
      ...
      if (!candidate) throw new Error('SA roster confirmation does not match a pending candidate');
      ...
      if (confirmedByTuple.has(key)) throw new Error('SA roster duplicate confirmation link');
      confirmedByTuple.set(key, link.localId);
    }

    const nextUsers: SaLocalEmployee[] = existingUsers.map(u => ({ ...u }));
    const byId = new Map<string, SaLocalEmployee>();
    for (const user of nextUsers) byId.set(user.id, user);

    let updatedCount = 0;
    let linkedCount = 0;

    function applyRecord(target: SaLocalEmployee, record: SaNormalizedRecord, raw: Record<string, unknown> | null): void {
      ...
      if (hasPaused) {
        if (record.paused === true) target.paused = true;
        else delete target.paused;
      }
      ...
      (target as Record<string, unknown>).schemaVersion = 1;
    }

    const recordIndexByTuple = new Map<string, number>();
    ...
    for (const update of plan.updates) {
      ...
      if (!target) throw new Error('SA roster update target is missing');
      const index = recordIndexByTuple.get(tupleKey(update.record.saProjectId, update.record.saEmployeeId)) ?? 0;
    ...
    for (const [key, localId] of confirmedByTuple) {
      ...
      if (!target) throw new Error('SA roster link target is missing');
      const record = safeRoster.employees.find(r => tupleKey(r.saProjectId, r.saEmployeeId) === key);
      if (!record) throw new Error('SA roster link record is missing');
      const index = recordIndexByTuple.get(key) ?? 0;
    ...
    for (const candidate of plan.reconciliationCandidates) {
      ...
      if (!confirmedByTuple.has(key)) unconfirmed.add(key);
    }
    ...
    const usedIds = new Set<string>(nextUsers.map(u => u.id));
    let createdCount = 0;
    const safeRosterIndex = new Map<string, number>();
    safeRoster.employees.forEach((record, index) => {
    ...
    for (const record of plan.creates) {
      ...
      if (unconfirmed.has(key)) continue;
      ...
      usedIds.add(localId);
      const index = safeRosterIndex.get(key) ?? 0;
      const raw = rawFor(record, index);
      const entry: SaLocalEmployee = {
        id: localId,
        name: record.name,
        number: record.number,
      ...
      (entry as Record<string, unknown>).schemaVersion = 1;
      (entry as Record<string, unknown>).localOnly = true;
      (entry as Record<string, unknown>).createdAt = now;
      (entry as Record<string, unknown>).updatedAt = now;
      nextUsers.push(entry);
      byId.set(localId, entry);
      createdCount += 1;
    }

    const remainingCandidates = plan.reconciliationCandidates.filter(candidate =>
      !confirmedByTuple.has(tupleKey(candidate.saProjectId, candidate.saEmployeeId))
    );
