...
(function exposeFieldRequests(root, factory) {
    const api = factory();
    if (typeof module === 'object' && module && module.exports)
        module.exports = api;
    if (root)
        root.FieldRequests = api;
    ...
    function resolveTargets(targetType, allEmployees = [], options) {
        ...
        if (targetType === 'position') {
            const posFilter = ((options === null || options === void 0 ? void 0 : options.targetPosition) || '').trim().toLowerCase();
            return allEmployees
        ...
        if (targetType === 'custom') {
            const idSet = new Set((options === null || options === void 0 ? void 0 : options.targetEmployeeIds) || []);
            return allEmployees
    ...
    function createRequest(params, allEmployees = [], nowFn = nowIso) {
        ...
        const employeePool = (Array.isArray(allEmployees) && allEmployees.length > 0)
            ? allEmployees
            : (Array.isArray(params.allEmployees) ? params.allEmployees : []);
        const targets = targetType !== 'none'
            ? resolveTargets(targetType, employeePool, {
                targetPosition: params.targetPosition,
                targetEmployeeIds: params.targetEmployeeIds || params.customRecipients
            })
            : [];
        const now = nowFn();
    ...
    function responseKey(fieldId, employeeId) {
        return employeeId ? `${fieldId}:${employeeId}` : fieldId;
    }
    function assertSafeId(value) {
        if (typeof value !== 'string' || !/^[A-Za-z0-9_-]+$/.test(value))
            throw new Error('Identificador de solicitud no seguro');
    }
    function normalizeRequest(value) {
        if (!value || typeof value !== 'object')
            throw new Error('Solicitud inválida');
        if (!value.id || !value.title || !Array.isArray(value.fields))
            throw new Error('Solicitud incompleta');
        assertSafeId(value.id);
        value.fields.forEach((field) => assertSafeId(field === null || field === void 0 ? void 0 : field.id));
        (value.targets || []).forEach((target) => assertSafeId(target === null || target === void 0 ? void 0 : target.id));
        ...
        source.forEach((raw) => {
            ...
            if (!raw || !raw.fieldId)
                return;
            const employeeId = (_b = (_a = raw.employeeId) !== null && _a !== void 0 ? _a : raw.recipientId) !== null && _b !== void 0 ? _b : null;
    ...
    function getResponse(request, fieldId, employeeId) {
        const responses = request === null || request === void 0 ? void 0 : request.responses;
        if (Array.isArray(responses)) {
            return responses.find(item => { var _a, _b; return (item === null || item === void 0 ? void 0 : item.fieldId) === fieldId && ((_b = (_a = item.employeeId) !== null && _a !== void 0 ? _a : item.recipientId) !== null && _b !== void 0 ? _b : null) === (employeeId !== null && employeeId !== void 0 ? employeeId : null); });
        }
        return responses === null || responses === void 0 ? void 0 : responses[responseKey(fieldId, employeeId)];
    }
    ...
    function isRequestComplete(request) {
        ...
        if (required.length === 0)
            return Object.keys(request.responses || {}).length > 0;
        if ((_a = request.targets) === null || _a === void 0 ? void 0 : _a.length) {
    ...
    function importRequestBackup(raw) {
        ...
        if ((parsed === null || parsed === void 0 ? void 0 : parsed.schemaVersion) !== 1)
            throw new Error('El respaldo no es compatible');
        if (!Array.isArray(parsed.requests) || !Array.isArray(parsed.templates))
        ...
        const templates = parsed.templates.map((template) => {
            if (!(template === null || template === void 0 ? void 0 : template.id) || !(template === null || template === void 0 ? void 0 : template.name) || !Array.isArray(template.fields))
                throw new Error('Plantilla inválida en el respaldo');
            return clone(template);
        });
        return { requests, templates };
    }
    function recordResponse(request, input, nowFn = nowIso) {
    ...
    function removeResponse(request, fieldId, employeeId, nowFn = nowIso) {
        const updated = normalizeRequest(request);
    ...
    function changeStatus(request, newStatus, nowFn = nowIso) {
        const updated = normalizeRequest(request);
        const now = nowFn();
    ...
    function recordShare(request, nowFn = nowIso) {
        const updated = normalizeRequest(request);
        const now = nowFn();
    ...
    function duplicateRequest(request, nowFn = nowIso, newTitle) {
        const now = nowFn();
        return {
            ...clone(request),
            id: generateId('req'),
    ...
    function formatWhatsAppSummary(request) {
        ...
        if (request.requiredAt || request.requiredDate)
            lines.push(`⏰ *Necesaria para:* ${formatDate(request.requiredAt || request.requiredDate)}`);
        if (request.category) {
        ...
        if (request.targetPosition)
            lines.push(`👷 *Puesto:* ${request.targetPosition}`);
        if (request.description)
            lines.push(`ℹ️ ${request.description}`);
        lines.push('─────────────────');
        if (request.targetType !== 'none' && summary.totalTargets > 0) {
            ...
            summary.fieldSummaries.forEach(fs => {
                if (fs.numericValues && fs.numericValues.length > 0) {
                    ...
                    if (val.comment)
                        lines.push(`   _Nota: ${val.comment}_`);
                }
    ...
    function formatWhatsAppDetail(request) {
        const summary = summarizeResults(request);
        const dateStr = formatDate(request.createdAt);
        const lines = [];
    ...
    function createTemplateFromRequest(request, name, isFavorite = false) {
    ...
    function updateTemplate(template, changes) {
    ...
    function filterRequests(requests = [], queryOrOptions = '', categoryFilter = 'all', statusFilter = 'all') {
        let queryStr = '';
        ...
        return (requests || []).filter(req => {
            ...
            if (statFilter === 'open')
                matchStatus = req.status === 'open' || req.status === 'draft';
            else if (statFilter === 'completed')
                matchStatus = req.status === 'completed';
            else if (statFilter === 'closed')
                matchStatus = req.status === 'closed';
            else if (statFilter !== 'all')
                matchStatus = req.status === statFilter;
