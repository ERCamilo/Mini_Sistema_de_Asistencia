...
(function exposeEmployeeNumberRules(root, factory) {
    ...
    if (root)
        root.EmployeeNumberRules = rules;
    ...
    function findEmployeeNumberConflict(users, candidateNumber, excludedEmployeeId) {
        ...
        if (normalizedCandidate === null)
            return null;
        return users.find(employee => (employee.id !== excludedEmployeeId
            && normalizeEmployeeNumber(employee.number) === normalizedCandidate)) || null;
    }
    ...
    function swapEmployeeNumbers(users, firstEmployeeId, secondEmployeeId) {
        ...
        if (!firstEmployee || !secondEmployee || firstEmployeeId === secondEmployeeId)
            return [...users];
        return users.map(employee => {
            if (employee.id === firstEmployeeId)
                return { ...employee, number: secondEmployee.number };
            if (employee.id === secondEmployeeId)
                return { ...employee, number: firstEmployee.number };
            return employee;
        });
    }
    ...
    function saveEmployeeDraft(users, draft, newEmployeeId) {
        ...
        if (employee)
            return { status: 'conflict', users, conflict: createConflict(users, draft, employee) };
